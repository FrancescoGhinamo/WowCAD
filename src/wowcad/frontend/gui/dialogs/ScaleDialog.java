...
public class ScaleDialog extends JDialog implements ActionListener {
	...
	private static final long serialVersionUID = -6603654247202216494L;
	...
	public ScaleDialog(JFrame owner, DialogCallback cbk, String[] names) {
		super(owner, "Scale", false);
	...
	private void initComponents(String[] names) {
		...
		JPanel cont = new JPanel(new GridLayout(2, 2, 5, 5));

		cont.add(new JLabel("Name"));
		cont.add(cmbName = new JComboBox<String>(names));
		cont.add(new JLabel("Scale factor"));
		cont.add(txtScale = new JTextField(20));
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource().equals(btnOK)) {
			if(!txtScale.getText().equals("")) {
				...
				command = CommandKeys.SCALE + CadManager.COMMAND_SPLITTER + (String)cmbName.getSelectedItem() + CadManager.PARAMETER_SPLITTER + txtScale.getText();
				dispose();
